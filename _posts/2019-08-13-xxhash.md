...
there's no obvious way to collect together what I will coin the "scrambling operations" - the multiplications and the shifts in this case - which means that one iteration of the loop must be evaluated before the next can start.
...
The implementation specifically targets a 128-but vectors, because it would be incorrect not to.
...
XXHash64 cannot be vectorised with AVX2, because there isn't a 64-bit vector multiplication in that instruction set, but the loads, additions and rotations can all be vectorised.
...
Of course, this is just data, and not reusable insight, but I'll come back to this topic another time.
