...
To write a BSON parser, you just need a jump table associating each value type with a parser callback.
As you scan the input, you read the type byte, read the attribute name, then look up the callback for the current type invoke it.
