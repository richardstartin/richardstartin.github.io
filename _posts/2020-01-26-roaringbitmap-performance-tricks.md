...
In all of these cases you beat `java.util.BitSet`, but this isn't true for all data sets.
...
I [wrote](/posts/garbage-collectors-affect-microbenchmarks) about how expensive G1 write barriers can be, after finding and removing a bottleneck in a parser I had implemented.
Coincidentally, Martin Thompson must have been profiling something similar, with exactly the same garbage collector settings as mine, and [reached]((https://github.com/real-logic/agrona/commit/532f97e31e47045983b528e700258b9d17b591e1)) the same conclusion; my findings were in good company.
...
It was a [big change](https://github.com/apache/druid/pull/6794) and there's a bit more to it than picking up batch iterators (I feel obliged to point out that I am English and this was understatement), but these iterators were one of the motivations, and the benefits for Druid users were quite large:
