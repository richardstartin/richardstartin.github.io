...
The idea is to split larger loop bodies which perform several distinct tasks into several loops, with the hope that the task loops can be optimised more effectively in isolation. 
As far as I'm aware, C2, Hotspot's JIT compiler, so you have to do it yourself.
I came across a couple of cases where this was profitable recently, and this post uses these cases as an example.
...
In a `RoaringBitmap` one of the [three container types](/posts/2017-03-01-a-quick-look-at-roaringbitmap.md) is a `BitmapContainer` which contains a bitmap, a dense collection of bits.
...
This can be done in one loop or two separate loops, as in the represenative benchmark below:
...
As can be seen above, loop fission is either a big win (lower score is better), or a loss depending on the ratio of the length to the number of groups.
When the ratio is high, the fissured implementation wins (see 4096/8 134ns vs 2479ns).
When the ratio is low, the opposite happens (see 512/128 1121ns vs 752ns).
